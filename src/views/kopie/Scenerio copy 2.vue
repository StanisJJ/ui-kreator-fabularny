      ...
      <h3>Draggable {{ draggingInfo }}</h3>
        ...
        >
          ...
          <li class="list-group-item" v-bind:style="{border: 'solid 1px' , 'border-color':  element.Color , 'background-color': element.Color + '70'}" v-for="(element) in Scenerios" :key="element.ID" >
            <div class="Image"><b-img class="Image" thumbnail :src="'http://localhost:5000/'+element.Image" alt="Image 1"></b-img></div>
            <div class="row">
              
                <b-icon class="handle col-1" scale="2" icon="grip-horizontal " ></b-icon>
                <div class="col-7 row Title Options" v-show="edit !== element.ID"><span class="col-2">Poz:{{element.Position }}</span><div class="col-10">{{ element.Title }}</div></div>
                <div class="col-7 row Title Options" v-show="edit === element.ID">
        ...
        event.preventDefault();
             ...
             if(this.file != null){
              ...
              // console.log(this.file.name);
                ...
                //if accept ext file
                    if(this.file.name == "" ){
                        this.fileError = "Uzupełnij wszystkie pola";
                    }
                    else if (this.file.name.length < 2 || this.file.name.length > 100)
                        this.fileError = "Nazwa powinna mieć od 2 do 30 znaków!";
                    else{
                        axios.post('http://localhost:5000/api/Upload/'+this.file.name,formData,{headers: {
                        authorization: localStorage.getItem('token'),
                        }} ).then(res => {
                            //if successfull
                            if(res.status === 200){
                                console.log(res);
                                this.NewScenerio.Image = res.data.Image;
                                console.log("--------------haklllaosdjk----",res.data.Image)
                                this.AddNew();
                            }
                            this.error = '';
                        },err => {
